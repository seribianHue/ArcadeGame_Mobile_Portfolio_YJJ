...
public class MapManager : MonoBehaviour
{
    ...
    void Start()
    {
        for(int i = 0; i < _planeArray.GetLength(0); i++)
        {
            for(int j = 0; j <  _planeArray.GetLength(1); j++)
            {
                ...
                CreatePlane(plane);
                plane.transform.position = new Vector3(-100 + (50 * j), 0, 100 - (50 * i));
            ...
            switch (state)
            {
                case PLAYERPOS.UP:
                    for(int i = 0; i < _planeArray.GetLength(0); i++)
                    {
                        ...
                        GameObject plane = Instantiate(_planePf, transform);
                        CreatePlane(plane);
                        plane.transform.position = new Vector3(_curPlayerX - 100 + (50 * i), 0, _curPlayerZ + 100);
                ...
                case PLAYERPOS.RIGHT:
                    for (int i = 0; i < _planeArray.GetLength(0); i++)
                    {
                        ...
                        GameObject plane = Instantiate(_planePf, transform);
                        CreatePlane(plane);
                        plane.transform.position = new Vector3(_curPlayerX + 100, 0, _curPlayerZ + 100 - (50 * i));
    ...
    void CreatePlane(GameObject plane)
    {
        ...
        for(int i = 0; i < numTree; i++)
        {
            ...
            GameObject tree = Instantiate(_treePf, plane.transform);
